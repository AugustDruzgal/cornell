...
static PT_THREAD (protothread_core_0(struct pt *pt))
{
    ...
    fix15 frequency_mult = int2fix15(1);
    fix15 step = float2fix15(0.1f);

    while(1) 
    {
        printf("Playing note\n");
        fflush(stdout);
        play_note(frequency_mult);

        frequency_mult += step;

        if (frequency_mult > float2fix15(1.5f))
        {
            frequency_mult = float2fix15(0.5f);
        }
        // Yield for 1s
        PT_YIELD_usec(10000000);
    }
    // Indicate thread end
    PT_END(pt);
}
...
int main() 
{
    ...
    {
        float frequency = (-260 * (float) sin((((float) -3.141592)/((float)NOTE_LEN))*x)) + 1740;
