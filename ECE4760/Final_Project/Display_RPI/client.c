...
static PT_THREAD (protothread_client(struct pt *pt))
{
    ...
    while(1) {
        ...
        for (counter = 0; counter < num_characteristics; counter++) {
            ...
            // printf("Value:\033[K\t\t\t %f\n\n", *((float *) server_characteristic_values[counter])) ;
            set_tilt_angle(*((float *) server_characteristic_values[counter]));

            if (!notifications_enabled[counter]) 
            {
                ...
                gatt_client_write_client_characteristic_configuration(handle_gatt_client_event, connection_handle,
                    &server_characteristic[counter], GATT_CLIENT_CHARACTERISTICS_CONFIGURATION_NOTIFICATION);
            }
        }
    }
